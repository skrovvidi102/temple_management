...
from pdf_utils import (generate_ticket_pdf, generate_donation_receipt, generate_stage_booking_receipt,
...
def staff_dashboard(root, username):
    ...
    tk.Label(header, text=f"{staff_name} • Staff", font=("Arial", 14, "bold"), bg="#f0f8ff").pack(side="left", padx=12)
    tk.Button(header, text="X", bg="#ef5350", fg="white", command=root.destroy).pack(side="right", padx=12)
    ...
    # --- actions used by staff ---
    def issue_ticket_ui():
        win = tk.Toplevel(root); win.title("Issue Ticket"); win.geometry("420x520"); win.configure(bg="#f5f5f5")
        tk.Label(win, text="Devotee Name:", bg="#f5f5f5").pack(pady=4)
        devotee_name = tk.Entry(win); devotee_name.pack(fill="x", padx=10)
        tk.Label(win, text="Contact:", bg="#f5f5f5").pack(pady=4)
        contact = tk.Entry(win); contact.pack(fill="x", padx=10)
        tk.Label(win, text="Worship Type:", bg="#f5f5f5").pack(pady=4)
        worship_types = get_all_worship_types()
        worship_cb = ttk.Combobox(win, values=[wt[1] for wt in worship_types]); worship_cb.pack(fill="x", padx=10)
        tk.Label(win, text="Number of Tickets:", bg="#f5f5f5").pack(pady=4)
        num_tickets = tk.Entry(win); num_tickets.pack(fill="x", padx=10)
        tk.Label(win, text="Transaction Type:", bg="#f5f5f5").pack(pady=4)
        transaction_type = tk.Entry(win); transaction_type.pack(fill="x", padx=10)
        ...
        def submit():
            name = devotee_name.get().strip(); contact_value = contact.get().strip()
            ...
            if not (name and worship):
                messagebox.showerror("Error", "Devotee name and worship type required"); return
            devotee_id = add_devotee(name, contact_value)
            rate = next((wt[2] for wt in worship_types if wt[1] == worship), 0.0)
            total_amount = num * rate
            ticket_id = issue_pooja_ticket(staff_id, devotee_id, worship, date.today().isoformat(), num, total_amount, ttype)
            messagebox.showinfo("Success", f"Ticket #{ticket_id} issued!\nTotal: {total_amount:.2f}")
            pdf_path = generate_ticket_pdf(ticket_id)
            win.destroy()
    ...
    def book_stage_ui():
        win = tk.Toplevel(root); win.title("Book Stage"); win.geometry("520x480"); win.configure(bg="#f5f5f5")
        tk.Label(win, text="Select Stage:", bg="#f5f5f5").pack(pady=6)
        stages = get_all_stages()
        stage_cb = ttk.Combobox(win, values=[f"{s[1]} ({s[2] or ''})" for s in stages]); stage_cb.pack(fill="x", padx=10)
        ...
        def submit_booking():
            ...
            if not selected:
                messagebox.showerror("Error", "Select a stage"); return
            stage_name = selected.split(" (")[0]
            stage_obj = next((s for s in stages if s[1] == stage_name), None)
            if not stage_obj:
                messagebox.showerror("Error", "Stage not found"); return
            stage_id = stage_obj[0]; stage_location = stage_obj[2]
            event_name = event_entry.get().strip()
            booking_date = date_entry.get().strip()
            start_time = start_entry.get().strip(); end_time = end_entry.get().strip()
            ...
            try:
                ...
                messagebox.showinfo("Success", f"Stage '{stage_name}' booked for {booking_date}")
                if bid:
                    generate_stage_booking_receipt(bid, stage_name, stage_location, event_name, booking_date, start_time, end_time, staff_name)
                win.destroy()
            except Exception as e:
                messagebox.showerror("Error", str(e))

        tk.Button(win, text="Book Stage", command=submit_booking, bg="#81c784").pack(pady=12)
        tk.Button(win, text="Back", command=win.destroy, bg="#e0e0e0").pack(side="bottom", pady=8)

    def view_stage_bookings():
        win = tk.Toplevel(root); win.title("Your Stage Bookings"); win.geometry("820x400"); win.configure(bg="#fafafa")
        tree = ttk.Treeview(win, columns=("BookingID","Event","Date","Start","End","Status"), show="headings")
        for col in ("BookingID","Event","Date","Start","End","Status"): tree.heading(col, text=col); tree.column(col,width=120)
        tree.pack(fill="both", expand=True, padx=10, pady=10)
        rows = get_stage_bookings_by_employee(staff_id)
        for r in rows:
            if len(r) >= 5:
                tree.insert("", "end", values=(r[0] if len(r)>0 else "", r[2] if len(r)>2 else "", r[3] if len(r)>3 else "", r[4] if len(r)>4 else "", r[5] if len(r)>5 else ""))
            else:
                tree.insert("", "end", values=r)
        tk.Button(win, text="Back", command=win.destroy, bg="#e0e0e0").pack(side="bottom", pady=8)

    def record_donation():
        win = tk.Toplevel(root); win.title("Record Donation"); win.geometry("420x320"); win.configure(bg="#f5f5f5")
        tk.Label(win, text="Donor Name:", bg="#f5f5f5").pack(pady=6); donor_name = tk.Entry(win); donor_name.pack(fill="x", padx=10)
        tk.Label(win, text="Amount:", bg="#f5f5f5").pack(pady=6); amount = tk.Entry(win); amount.pack(fill="x", padx=10)
        tk.Label(win, text="Donation Type:", bg="#f5f5f5").pack(pady=6); dtype = tk.Entry(win); dtype.pack(fill="x", padx=10)
        def submit():
            ...
            record_donation(staff_id, donor_id, date.today().isoformat(), dtype.get().strip(), amt)
            conn = get_connection(); cur = conn.cursor()
    ...
    def view_tickets():
        win = tk.Toplevel(root); win.title("Issued Tickets"); win.geometry("700x420"); win.configure(bg="#fafafa")
        tree = ttk.Treeview(win, columns=("TicketID","Worship","Date","Tickets","Total"), show="headings")
        for col in ("TicketID","Worship","Date","Tickets","Total"): tree.heading(col, text=col); tree.column(col,width=120)
        tree.pack(fill="both", expand=True, padx=10, pady=10)
    ...
    def view_festival():
        win = tk.Toplevel(root); win.title("Festival Calendar"); win.geometry("600x450"); win.configure(bg="#f5f5f5")
        tree = ttk.Treeview(win, columns=("ID","Name","Date","Notes"), show="headings")
        for col in ("ID","Name","Date","Notes"): tree.heading(col, text=col)
        tree.pack(fill="both", expand=True, padx=10, pady=10)
        for fest in get_all_festivals(): tree.insert("", "end", values=fest)
        def print_month():
            month = simpledialog.askinteger("Month","Enter month (1-12)")
            year = simpledialog.askinteger("Year","Enter year (e.g., 2025)")
            if not (month and year): messagebox.showerror("Error","Month and Year required"); return
            monthly = get_festivals_by_month(year, month)
            generate_festival_calendar_pdf(monthly, year, month)
            messagebox.showinfo("Printed", "Monthly festival calendar generated")
    ...
    def view_last_receipt():
        ...
        if not last:
            messagebox.showinfo("Info","No recent ticket found"); return
    ...
    cards = [
        ...
        ("📅","My Bookings","View your stage bookings", view_stage_bookings),
        ("💰","Record Donation","Record & receipt", record_donation),
        ("📋","My Tickets","View tickets you issued", view_tickets),
        ("🧾","Last Receipt","View last ticket receipt", view_last_receipt),
        ("📅","Festival Calendar","View & print monthly", view_festival),
